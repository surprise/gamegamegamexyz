...
var timesArray = JSON.parse(loadLocal("colltimesarray", "[]"));
...
function showScramble()
{
    ...
    if (window.selCases.length == 0) {
        s = "click \"select cases\" above and pick some COLLs to practice";
...
function generateScramble()
{
    ...
    //         " <span onclick='displayBox(event,"+lastCase+")' class='collNameStats'>("+algsInfo[lastCase]["name"] + ") </span><a class='settings' onclick='confirmUnsel("+lastCase+")' style='color:"+document.getElementById( "linkscolor_in" ).value+";'>unselect</a>";
    displayPracticeInfo();
    // get random case
    var caseNum = 0;
    ...
    var alg = inverse_scramble(randomElement(window.collMap[caseNum]));
...
function displayBox(event,i) {
    ...
    //document.getElementById("boxsetup").innerHTML = "Setup: " + window.collMap[i][0];
...
{
    saveLocal("colltimesarray", JSON.stringify(window.timesArray));
    ...
    if (displayByCases)
    {
        ...
        for (var i = 0; i < len; i++)
        {
            var currentCOll = window.timesArray[i]["case"];
            if (resultsByCase[currentCOll] == null)
                resultsByCase[currentCOll] = [];
            resultsByCase[currentCOll].push( window.timesArray[i] );
        }
        ...
        for (var j = 0; j < keys.length; j++) {
            ...
            var meanForCase = 0.0;
            var i = 0;
            for (; i < resultsByCase[coll].length; i++)
            {
                timesString += makeHtmlDisplayableTime(resultsByCase[coll][i]);
                if (i != resultsByCase[coll].length - 1)
                    timesString += ", ";
                // avg
                meanForCase *= i/(i+1);
                meanForCase += resultsByCase[coll][i]["ms"] / (i+1);
            }
            s += "<div class='collNameHeader' " +
            ")'><span class='collNameStats' onclick='displayBox(event,"+keys[j]+")'>" + algsInfo[coll]["name"] + "</span>: " + msToHumanReadable(meanForCase) + "</div>" + timesString + "<br><br>";
        }
        el.innerHTML = s;
    }
    else
