...
var timesArray = JSON.parse(loadLocal("cmlltimesarray", "[]"));
...
function showScramble()
{
    ...
    if (window.selCases.length == 0) {
        s = "click \"select cases\" above and pick some CMLLs to practice";
...
function generateScramble()
{
    ...
    //         " <span onclick='displayBox(event,"+lastCase+")' class='cmllNameStats'>("+algsInfo[lastCase]["name"] + ") </span><a class='settings' onclick='confirmUnsel("+lastCase+")' style='color:"+document.getElementById( "linkscolor_in" ).value+";'>unselect</a>";
    displayPracticeInfo();
    // get random case
    var caseNum = 0;
    ...
    var alg = inverse_scramble(randomElement(window.cmllMap[caseNum]));
...
function displayBox(event,i) {
    ...
    //document.getElementById("boxsetup").innerHTML = "Setup: " + window.cmllMap[i][0];
...
{
    saveLocal("cmlltimesarray", JSON.stringify(window.timesArray));
    ...
    if (displayByCases)
    {
        ...
        for (var i = 0; i < len; i++)
        {
            var currentCmll = window.timesArray[i]["case"];
            if (resultsByCase[currentCmll] == null)
                resultsByCase[currentCmll] = [];
            resultsByCase[currentCmll].push( window.timesArray[i] );
        }
        ...
        for (var j = 0; j < keys.length; j++) {
            ...
            var meanForCase = 0.0;
            var i = 0;
            for (; i < resultsByCase[cmll].length; i++)
            {
                timesString += makeHtmlDisplayableTime(resultsByCase[cmll][i]);
                if (i != resultsByCase[cmll].length - 1)
                    timesString += ", ";
                // avg
                meanForCase *= i/(i+1);
                meanForCase += resultsByCase[cmll][i]["ms"] / (i+1);
            }
            s += "<div class='cmllNameHeader' " +
            ")'><span class='cmllNameStats' onclick='displayBox(event,"+keys[j]+")'>" + algsInfo[cmll]["name"] + "</span>: " + msToHumanReadable(meanForCase) + "</div>" + timesString + "<br><br>";
        }
        el.innerHTML = s;
    }
    else
